...
function scoreProduct(query, product = {}) {
  ...
  const queryWantsAccessory = /\b(case|cover|protector|glass|charger|cable|screen|lens|battery|adapter|remote|control|connector|replacement|keyboard|caps|stand|mount|bag|sleeve|interface)\b/i.test(normalizedQuery);
  const queryWantsCoreDevice = /\b(iphone|galaxy|pixel|phone|smartphone|laptop|notebook|computer|tv|television|monitor|tablet|camera)\b/i.test(normalizedQuery);
  const accessoryTitle = /\b(case|cover|protector|glass|charger|cable|screen|lens|film|tempered|hydrogel|remote|control|connector|adapter|replacement|keyboard|caps|stand|mount|bag|sleeve|interface)\b/i.test(title);
  ...
  const relevant = matchedTerms.length >= requiredMatches && score >= (terms.length <= 2 ? 16 : 22) && (!queryIsPhoneModel || phoneModelMatched) && !blockedAccessory;
